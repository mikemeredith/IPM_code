...
boxplot(s, ylab="Realized proportion of surviving birds",
    xlab="Population size (number of females)", names=size,
    outline=FALSE, col="dodgerblue", las=1, frame=FALSE)
...
plot(N[1,], type="l", lwd=1.5, ylab="Population size", xlab="Time",
    # axes=FALSE, ylim=range(N))
    axes=FALSE, ylim=range(N, na.rm=TRUE))
axis(1)
axis(2, las=1)
