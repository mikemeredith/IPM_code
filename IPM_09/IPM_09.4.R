...
mu <- list(phij=draws$mean.phij, phia=draws$mean.phia, om=draws$mean.omega,
    f1=draws$mean.f1, f2=draws$mean.f2, n1=apply(n1, 1, mean),
...
means_phij <- means_phia <- means_f1 <- means_f2 <- means_om <- means_n1 <-
    means_n2 <- means_n3 <- matrix(NA, nrow=n.draws, ncol=n.years-1)
...
means <- list(phij=getMn(draws$phij), phia=getMn(draws$phia), om=getMn(draws$om),
    f1=getMn(draws$f1), f2=getMn(draws$f2), n1=getMn(draws$N[,1,]),
    n2=getMn(draws$N[,2,]), n3=getMn(draws$N[,3,]))
