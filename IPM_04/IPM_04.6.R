...
for (i in 1:nrow(y)){         # Loop over individuals
  plot(traplocs, xlim=c(Xl, Xu), ylim=c(Yl, Yu), pch=16, xlab='x-coordinate', ylab='y-coordinate', frame=FALSE)
...
parameters <- c("alpha0", "alpha1", "sigma", "N", "D", "s", "z", "r.95", "A.95")
...
image(xg, yg, D.hat1, col=cl, xlab='x-coordinate', ylab='y-coordinate',
    asp=1, main='Density based on home range centers')
...
# Make a plot of that density table
image(xg, yg, D.hat2, col=cl, xlab='x-coordinate', ylab='y-coordinate',
    asp=1, main='Density based on animal locations')
points(Sobs, col='red', pch=16)
points(traplocs, pch=16)
points(S, col='red')
