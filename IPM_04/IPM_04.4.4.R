...
table(C3)                    # Remind ourselves of frequency distribution
...
cbind('Truth'=C, 'Obs w/cens'=C3, 'Modelled data (C4)'=C4,
    'Cens. indicator (d)'=d, cens.threshold)[1:6,]   # Look at first 6 rows
...
out14 <- jags(jags.data, inits, parameters, "model11.txt",
    n.iter=ni, n.burnin=nb, n.chains=nc, n.thin=nt, n.adapt=na, parallel=TRUE)
op <- par(mfrow=c(2, 2));  traceplot(out14)    # Not shown
par(op)
print(out14, 3)
...
model {
  ...
  # Second, integrated model: Poisson GLM for C5 (part 1)
  #   plus a Bernoulli GLM for C5 (part 2) with appropriate definition
  #   of the success probability as a function of the same rho
...
# Call JAGS from R (ART <1 min), check convergence and summarize posteriors
out15 <- jags(jags.data, inits, parameters, "model12.txt",
    n.iter=ni, n.burnin=nb, n.chains=nc, n.thin=nt, n.adapt=na, parallel=TRUE)
op <- par(mfrow=c(2, 2));  traceplot(out15)    # Not shown
par(op)
print(out15, 3)
