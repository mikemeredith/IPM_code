...
trueN <- apply(z, 2, sum)
# Obs. population size if you had visited each territory twice a year
obsN1 <- apply(apply(y1, c(1, 3), max), 2, sum)
# Obs. population size if you not had visited each territory twice a year
...
model {
  ...
  for (t in 1:nyear){
    ntot[t] <- sum(z[,t])    # Number of occupied territories = pop. size
  }
}
")
