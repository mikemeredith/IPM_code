...
set.seed(1982)
str(MHBdata <- simMHB(nsites=267, nsurveys=3, nyears=25, mean.lam=1,
    mean.beta=0.03, sd.lam=c(0.5, 0.05), mean.p=0.6, beta.p=0, show.plot=TRUE))
...
jags.data <- with(MHBdata, (list(C=C, nsites=dim(C)[1],
    nsurveys=dim(C)[2], nyears=dim(C)[3])))
str(jags.data)
...
model {
  ...
  for (t in 1:nyears){
    totalN[t] <- sum(N[,t])
    logtotalN[t] <- log(sum(N[,t]))   # Check that out as well
  }
  }
")
...
# parameters <- c("lambda", "p", "totalN", "logtotalN")
...
# Parameters monitored
parameters <- c("lambda", "mu.lam", "sig2.ystar", "sig2.lam", "sig.ystar",
    "sig.lam", "N", "ystar")
...
# Parameters monitored
parameters <- c("lambda", "mu.lam", "sig2.ystar", "sig2.lam",
    "sig.ystar", "sig.lam", "N", "ystar")
...
print(cbind('pm (model 2)'=out5$mean$N, 'psd (model 2)'=out5$sd$N,
    'pm (model 3)'=out6$mean$N, 'psd (model 3)'=out6$sd$N), 3)
