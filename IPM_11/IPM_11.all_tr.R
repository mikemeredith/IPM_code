...
par(las=1, cex=1.25)
barplot(t(table(woodchat11$f, woodchat11$d)), xlab = "Number of fledglings per brood", ylab = "Number of broods", col = co, border = NA)
legend("topright", legend=c("Censored", "Accurate"), col=rev(co), bty="n", pch=c(15,15), pt.cex=2)
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
par(mfrow=c(1,3), las=1, mar=c(4.1, 4.5, 3, 2))
z <- range(c(out1$sims.list$fit.C, out1$sims.list$fitN.C))
z <- c(10, 50)
plot(x=out1$sims.list$fit.C, y=out1$sims.list$fitN.C, pch=16, ylim=z, xlim=z, ylab="Discrepancy replicate data", xlab=" Discrepancy actual data", main="Population count data", col=rgb(0, 0, 0, 0.3), axes=FALSE, cex=0.8)
segments(z[1], z[1], z[2], z[2])
text(x=z[2], y=z[1], paste("P=", round(mean(out1$sims.list$fit.C < out1$sims.list$fitN.C), 2)), adj=c(1,0))
axis(1); axis(2)

z <- range(c(out1$sims.list$fit.f, out1$sims.list$fitN.f))
plot(x=out1$sims.list$fit.f, y=out1$sims.list$fitN.f, pch=16, ylim=z, xlim=z, ylab="", xlab=" Discrepancy actual data", main="Productivity data", col=rgb(0, 0, 0, 0.3), axes=FALSE, cex=0.8)
segments(z[1], z[1], z[2], z[2])
text(x=z[2], y=z[1], paste("P=", round(mean(out1$sims.list$fit.f < out1$sims.list$fitN.f), 2)), adj=c(1,0))
axis(1); axis(2)

z <- range(c(out1$sims.list$fit.CJS, out1$sims.list$fitN.CJS))
plot(x=out1$sims.list$fit.CJS, y=out1$sims.list$fitN.CJS, pch=16, ylim=z, xlim=z, ylab="", xlab=" Discrepancy actual data", main="Capture-recapture data", col=rgb(0, 0, 0, 0.3), axes=FALSE, cex=0.8)
segments(z[1], z[1], z[2], z[2])
text(x=z[2], y=z[1], paste("P=", round(mean(out1$sims.list$fit.CJS < out1$sims.list$fitN.CJS), 2)), adj=c(1,0))
axis(1); axis(2)
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
plot(x=(1:28)+0.5, y=out1$mean$phij, type="b", pch=16, ylim=c(0, 0.8), ylab=expression(paste("Juvenile survival (", phi[italic(j)], ")")), xlab=NA, axes=F)
...
plot(x=(1:28)+0.5, y=out1$mean$phia, type="b", pch=16, ylim=c(0, 0.8), ylab=expression(paste("Adult survival (", phi[italic(a)], ")")), xlab=NA, axes=F)
segments((1:28)+0.5, out1$q2.5$phia, (1:28)+0.5, out1$q97.5$phia)
axis(1, at=1:29, tcl=-0.25, label=NA)
axis(1, at=c(2, 7, 12, 17, 22, 27), label=time)
axis(2, las=1)

plot(out1$mean$nu, type="b", pch=16, ylim=c(0, 1), ylab=expression(paste("Nest success probability (", nu, ")")), xlab=NA, axes=F)
segments(1:29, out1$q2.5$nu, 1:29, out1$q97.5$nu)
axis(1, at=1:29, tcl=-0.25, label=NA)
axis(1, at=c(2, 7, 12, 17, 22, 27), label=time)
axis(2, las=1)

plot(out1$mean$kappa, type="b", pch=16, ylim=c(0, 5.5), ylab=expression(paste("Conditional productivity (", kappa, ")")), xlab=NA, axes=F)
segments(1:29, out1$q2.5$kappa, 1:29, out1$q97.5$kappa)
axis(1, at=1:29, tcl=-0.25, label=NA)
axis(1, at=c(2, 7, 12, 17, 22, 27), label=time)
axis(2, las=1)

plot(x=(1:28)+0.5, y=out1$mean$omega.rate, type="b", pch=16, ylim=range(c(out1$q2.5$omega.rate, out1$q97.5$omega.rate)), ylab= "Immigration rate", xlab=NA, axes=F)
segments((1:28)+0.5, out1$q2.5$omega.rate, (1:28)+0.5, out1$q97.5$omega.rate)
axis(1, at=1:29, tcl=-0.25, label=NA)
axis(1, at=c(2, 7, 12, 17, 22, 27), label=time)
axis(2, las=1)

plot(out1$mean$rho, type="b", pch=16, ylim=c(0, 5.5), ylab=expression(paste("Total productivity (", rho, ")")), xlab=NA, axes=F)
...
legend("topright", pch = rep(15,3), legend = c('Immigrants', 'Local recruits', 'Surviving adults'), col = rev(co), bty = 'n')
...
rel.j <- rowSums(marr[,,1])
rel.a <- rowSums(marr[,,2])
recap <- c(1, 2, 3, 4, 5, 6, 7, 8, 19, 9, 10, 11, 12, 19, 19, 19, 19, 13, 14, 19, 19, 19, 19, 19, 15, 16, 17, 18)

jags.data <- list(n.years=dim(marr)[2], marr.j=marr[,,1], marr.a=marr[,,2], rel.j=rel.j, rel.a=rel.a, recap=recap, n.recap=max(recap), f=woodchat11$f, d=woodchat11$d, censored=censored, year=woodchat11$year-1963, C=woodchat11$count, pNinit=dUnif(1, 20))
...
# Parameters monitored
parameters <- c("mean.phij", "mean.phia", "mean.nu", "mean.kappa", "mean.omega", "sigma.phij", "sigma.phia", "sigma.nu", "sigma.kappa", "sigma.omega", "sigma.f", "phij", "phia", "nu", "kappa", "omega", "R", "S", "I", "N", "mean.p")
...
# Parameters monitored
parameters <- c("mean.phij", "mean.phia", "mean.nu", "mean.kappa", "mean.omega",  "sigma.nu", "sigma.kappa",  "sigma.f", "phij", "phia", "nu", "kappa", "omega", "R", "S", "I", "N", "mean.p")
...
par(mfrow=c(3,1), mar=c(3, 4.5, 2, 0.5))
k <- length(out1$mean$phia)
plot(x=(1:k)+0.5, y=out1$mean$phia, type="b", ylim=c(0, 1), pch=16, ylab=expression(paste("Estimated adult survival probability (", phi[italic(a)], ")")), col=co[1], axes=FALSE, xlab=NA)
legend(y=1, x=20, pch=rep(16, 2), col=co[1:2], legend=c(expression(IPM[1]), expression(IPM[2])), bty="n")
legend(y=1, x=24, pch=rep(16, 2), col=co[3:4], legend=c(expression(IPM[3]), expression(IPM[4])), bty="n")
axis(2, las=1)
axis(1, at=1:29, tcl=-0.25, label=NA)
axis(1, at=c(2, 7, 12, 17, 22, 27), label=time)
polygon(x=c(1:k,k:1)+0.5, y=c(out1$q2.5$phia, rev(out1$q97.5$phia)), col=alpha(co[1], alpha=al[1]), border=NA)
polygon(x=c(1:k,k:1)+0.5, y=c(out2$q2.5$phia, rev(out2$q97.5$phia)), col=alpha(co[2], alpha=al[2]), border=NA)
polygon(x=c(1:k,k:1)+0.5, y=c(out3$q2.5$phia, rev(out3$q97.5$phia)), col=alpha(co[3], alpha=al[3]), border=NA)
polygon(x=c(1:k,k:1)+0.5, y=c(out4$q2.5$phia, rev(out4$q97.5$phia)), col=alpha(co[4], alpha=al[4]), border=NA)
...
plot(x=(1:k)+0.5, y=real.phia(out1)$m.phia, type="b", ylim=c(0, 1), pch=16, ylab=expression(paste("Realized adult survival (", italic(S)[italic(t)+1], " / ", italic(N)[italic(t)], ")")), col=co[1], axes=FALSE, xlab=NA)
axis(2, las=1)
axis(1, at=1:29, tcl=-0.25, label=NA)
axis(1, at=c(2, 7, 12, 17, 22, 27), label=time)
polygon(x=c(1:k,k:1)+0.5, y=c(real.phia(out1)$l.phia, rev(real.phia(out1)$u.phia)), col=alpha(co[1], alpha=al[1]), border=NA)
polygon(x=c(1:k,k:1)+0.5, y=c(real.phia(out2)$l.phia, rev(real.phia(out2)$u.phia)), col=alpha(co[2], alpha=al[2]), border=NA)
polygon(x=c(1:k,k:1)+0.5, y=c(real.phia(out3)$l.phia, rev(real.phia(out3)$u.phia)), col=alpha(co[3], alpha=al[3]), border=NA)
polygon(x=c(1:k,k:1)+0.5, y=c(real.phia(out4)$l.phia, rev(real.phia(out4)$u.phia)), col=alpha(co[4], alpha=al[4]), border=NA)
...
plot(x=1:k, y=out1$mean$S, type="b", ylim=c(0, 20), pch=16, ylab=expression("Number of survivors ("*italic(S)*")"), axes=FALSE, xlab=NA)
...
plot(x=1:k, y=out1$mean$omega, type="b", ylim=c(0, 25), pch=16, ylab=expression(paste("Expected number of immigrants (", Omega, ")")), col=co[1], axes=FALSE, xlab=NA)
legend(y=25, x=20, pch=rep(16, 2), col=co[1:2], legend=c(expression(IPM[1]), expression(IPM[2])), bty="n")
legend(y=25, x=24, pch=rep(16, 2), col=co[3:4], legend=c(expression(IPM[3]), expression(IPM[4])), bty="n")
axis(2, las=1)
axis(1, at=1:29, tcl=-0.25, label=NA)
axis(1, at=c(2, 7, 12, 17, 22, 27), label=time)
polygon(x=c(1:k,k:1), y=c(out1$q2.5$omega, rev(out1$q97.5$omega)), col=alpha(co[1], alpha=al[1]), border=NA)
polygon(x=c(1:k,k:1), y=c(out2$q2.5$omega, rev(out2$q97.5$omega)), col=alpha(co[2], alpha=al[2]), border=NA)
polygon(x=c(1:k,k:1), y=c(out3$q2.5$omega, rev(out3$q97.5$omega)), col=alpha(co[3], alpha=al[3]), border=NA)
polygon(x=c(1:k,k:1), y=c(out4$q2.5$omega, rev(out4$q97.5$omega)), col=alpha(co[4], alpha=al[4]), border=NA)
...
plot(x=1:k, y=out1$mean$I, type="b", ylim=c(0, 25), pch=16, ylab=expression("Realized number of immigrants ("*italic(I)*")"), axes=FALSE, xlab=NA)
...
plot(x=1:k, y=out1$mean$N, type="b", ylim=c(0, 35), pch=16, ylab=expression("Number of breeders ("*italic(N)*")"), axes=FALSE, xlab=NA)
