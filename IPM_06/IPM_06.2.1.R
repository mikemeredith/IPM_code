...
parameters.ipm <- c("mean.sj", "mean.sa", "mean.p", "mean.f", "N", "sigma", "ann.growth.rate", "Ntot")
...
for (s in 1:nsim){
  ...
  jags.data.ipm <- list(marr.j=marr[,,1], marr.a=marr[,,2], n.occasions=T, rel.j=rowSums(marr[,,1]), rel.a=rowSums(marr[,,2]), sJ=sJ, nJ=nJ, C=count)
  jags.data.cjs <- list(marr.j=marr[,,1], marr.a=marr[,,2], n.occCJS=T, rel.j=rowSums(marr[,,1]), rel.a=rowSums(marr[,,2]))
  jags.data.pois <- list(sJ=sJ, nJ=nJ)
  jags.data.ssm <- list(n.occPop=T, C=count)

  # 3.7. Call JAGS from R (jagsUI) to run the 4 models
  m1 <- try(jags(jags.data.ipm, inits.ipm, parameters.ipm, "model6.txt", n.iter=ni, n.burnin=nb, n.chains=nc, n.thin=nt, n.adapt=na, parallel=TRUE))
  if(!inherits(m1, "try-error")) res1[,,s] <- m1$summary

  m2 <- try(jags(jags.data.cjs, inits.cjs, parameters.cjs, "model7.txt", n.iter=ni, n.burnin=nb, n.chains=nc, n.thin=nt, n.adapt=na, parallel=TRUE))
  if(!inherits(m2, "try-error")) res2[,,s] <- m2$summary

  m3 <- try(jags(jags.data.pois, inits.pois, parameters.pois, "model8.txt", n.iter=ni, n.burnin=nb, n.chains=nc, n.thin=nt, n.adapt=na, parallel=TRUE))
  if(!inherits(m3, "try-error")) res3[,,s] <- m3$summary

  m4 <- try(jags(jags.data.ssm, inits.ssm, parameters.ssm, "model9.txt", n.iter=ni, n.burnin=nb, n.chains=nc, n.thin=nt, n.adapt=na, parallel=TRUE))
  if(!inherits(m4, "try-error")) res4[,,s] <- m4$summary
